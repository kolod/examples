...
*/
QCPMarginGroup::QCPMarginGroup(QCustomPlot *parentPlot) : QObject(parentPlot),
...
*/
QCPLayoutElement::QCPLayoutElement(QCustomPlot *parentPlot) : QCPLayerable(parentPlot), // parenthood is changed as soon as layout element gets inserted into a layout (except for top level layout)
...
{
  if ((phase == upMargins) && (mAutoMargins != QCP::msNone)) {
...
*/
QCPLayoutGrid::QCPLayoutGrid() : mColumnSpacing(5),
                                 mRowSpacing(5),
                                 mWrap(0),
  ...
  for (int i = 0; i < mElements.size(); ++i)
  {
    ...
    else if (mInsetPlacement.at(i) == ipBorderAligned)
    {
      ...
      if (al.testFlag(Qt::AlignLeft))
        ...
        insetRect.moveLeft(int(rect().x() + rect().width() * 0.5 - finalMinSize.width() * 0.5)); // default to Qt::AlignHCenter
      if (al.testFlag(Qt::AlignTop))
        ...
        insetRect.moveTop(int(rect().y() + rect().height() * 0.5 - finalMinSize.height() * 0.5)); // default to Qt::AlignVCenter
    }
