...
QByteArray QCPLabelPainterPrivate::generateLabelParameterHash() const
{
  ...
  result.append(QByteArray::number(static_cast<int>(mSubstituteExponent)));
...
QByteArray QCPLabelPainterPrivate::cacheKey(const QString &text, const QColor &color, double rotation, AnchorSide side) const
{
  return text.toUtf8()+
      ...
      QByteArray::number(color.alpha()+256*static_cast<int>(side), 36)+
      QByteArray::number((static_cast<int>(rotation)*100)%36000, 36);
}
